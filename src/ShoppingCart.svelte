<script lang="ts">
	...
	function addItemToCart() {
		cartItemsStore.update((data) => mergeItem(data, newItem))
	}
	const shuffle = () => {
		const shuffled = cartItems
			.map((value) => ({ value, sort: Math.random() }))
			.sort((a, b) => a.sort - b.sort)
			.map(({ value }) => value)
		cartItems = shuffled
		$cartItemsStore = shuffled
	}
</script>

<div
	transition:fade
	class="z-50 overflow-hidden flex justify-end w-full absolute inset-0 bg-black/50"
>
